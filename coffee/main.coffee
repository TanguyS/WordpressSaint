...
jQuery(document).ready ($) ->
	...
	$(document).on "click", ->
		# do nothing, it resolves the "non-clicking bug" in iOs????
	...
	setSizes = ->
		window.screenWidth = (if window.innerWidth then window.innerWidth else $(window).width())
	...
	window.isTablet = navigator.userAgent.match(/iPad/i) != null || (navigator.userAgent.match(/Android/i) && (window.screenWidth > 899 || @screenHeight > 899));

	window.isHandled = window.isMobile or window.isTablet
